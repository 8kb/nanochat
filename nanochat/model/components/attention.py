...
class CausalSelfAttention(nn.Module):
    ...
    producer's kv_slot explicitly by the model that wires them together (see
    nanochat.model.llama_kvshare). Passing the producer's own k/v tensors back into
    flash_attn_with_kvcache for the consumer (rather than k=None) sidesteps a real FA3-vs-SDPA
    divergence in what k=None means (see docs/architecture.md) -- the write is a no-op since the
